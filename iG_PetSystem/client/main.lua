...
function OpenPetMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'pet_menu', {
		...
		if data.current.value == 'come_animal' and come == 0 then
			ESX.TriggerServerCallback('iG_PetSystem:getPet', function(pet)
				if pet == 'chien' then
					model = -1788665315
					come = 1
					openchien()
				...
				elseif pet == 'rottweiler' then
					...
					print('iG_PetSystem: unknown pet ' .. pet)
				end
			end)
			menu.close()
			...
			if distance < 5 then
				if count >= 1 then
					if status < 100 then
						...
						TriggerServerEvent('iG_PetSystem:consumePetFood')
						if status > 100 then
							status = 100
						end
						menu.close()
...
Citizen.CreateThread(function()
	while true do
		...
		if status == 0 then
			TriggerServerEvent('iG_PetSystem:petDied')
			DeleteEntity(ped)
...
function OpenPetShop()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'pet_shop', {
		...
		ESX.TriggerServerCallback('iG_PetSystem:buyPet', function(boughtPed)
			if boughtPed then
				menu.close()
			end
