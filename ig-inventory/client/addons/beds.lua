RegisterNetEvent("ig-inventory:openMotelsInventoryBed")
AddEventHandler("ig-inventory:openMotelsInventoryBed", function(data)
...
RegisterNUICallback("PutIntoMotelBed", function(data, cb)
	...
	if type(data.number) == "number" and math.floor(data.number) == data.number then
		...
		if data.item.name == 'cash' then
			TriggerServerEvent("ig-motels:putItemBed", ESX.GetPlayerData().identifier, 'item_account', 'money', count)
		else
			TriggerServerEvent("ig-motels:putItemBed", ESX.GetPlayerData().identifier, data.item.type, data.item.name, count)
		end
	end
	Wait(150)
