...
RegisterNetEvent("ig-inventory:openPlayerInventory")
AddEventHandler("ig-inventory:openPlayerInventory", function(target, playerName)
	targetPlayer = target
	targetPlayerName = playerName
	setPlayerInventoryData()
	openPlayerInventory()
	TriggerServerEvent('ig-inventory:disableTargetInv', target)
end)
					...
					if (Config.AllowPolice and PlayerData.job.name == Config.InventoryJob.Police) or (Config.AllowNightclub and PlayerData.job.name == Config.InventoryJob.Nightclub) or (Config.AllowMafia and PlayerData.job.name == Config.InventoryJob.Mafia) or (Config.AllowMafia and PlayerData.job.name == Config.InventoryJob.Ambulance) then
						...
						if IsEntityPlayingAnim(searchPlayerPed, 'random@mugging3', 'handsup_standing_base', 3) or IsEntityPlayingAnim(searchPlayerPed, 'missminuteman_1ig_2', 'handsup_base', 3) or IsEntityDead(searchPlayerPed) or GetEntityHealth(searchPlayerPed) <= 0 or IsEntityPlayingAnim(searchPlayerPed, "mp_arresting", "idle", 3) or IsEntityPlayingAnim(searchPlayerPed, "mp_arrest_paired", "crook_p2_back_right", 3) then
							exports['mythic_progbar']:Progress({
								...
								if not status then
									TriggerEvent("ig-inventory:openPlayerInventory", GetPlayerServerId(closestPlayer), GetPlayerName(closestPlayer))
								end
							end)
						else
							exports['mythic_notify']:SendAlert('error', _U('player_not_dead'))
						end
					else
						exports['mythic_notify']:SendAlert('error', _U('no_permissions'))
					end
				else
					local searchPlayerPed = GetPlayerPed(closestPlayer)
					if IsEntityPlayingAnim(searchPlayerPed, 'random@mugging3', 'handsup_standing_base', 3) or IsEntityPlayingAnim(searchPlayerPed, 'missminuteman_1ig_2', 'handsup_base', 3) or IsEntityDead(searchPlayerPed) or GetEntityHealth(searchPlayerPed) <= 0 or IsEntityPlayingAnim(searchPlayerPed, "mp_arresting", "idle", 3) or IsEntityPlayingAnim(searchPlayerPed, "mp_arrest_paired", "crook_p2_back_right", 3) then
						exports['mythic_progbar']:Progress({
							name = "openGlovebox",
							duration = 3500,
							label = 'You begin searching the individual...',
							useWhileDead = false,
							canCancel = true,
							controlDisables = {},
							animation = {},
						...
						exports['mythic_notify']:SendAlert('error', 'There is no one dead nearby...')
						Citizen.Wait(2000)
					end
				end
            end
...
RegisterNUICallback("PutIntoPlayer", function(data, cb)
	...
	if type(data.number) == "number" and math.floor(data.number) == data.number then
		...
		TriggerServerEvent("ig-inventory:tradePlayerItem", GetPlayerServerId(PlayerId()), targetPlayer, data.item.type, data.item.name, count)
	end
	Wait(150)
...
RegisterNUICallback("TakeFromPlayer", function(data, cb)
	...
	if type(data.number) == "number" and math.floor(data.number) == data.number then
		...
		TriggerServerEvent("ig-inventory:tradePlayerItem", targetPlayer, GetPlayerServerId(PlayerId()), data.item.type, data.item.name, count)
	end
	Wait(150)
	refreshPlayerInventory()
	loadPlayerInventory()
