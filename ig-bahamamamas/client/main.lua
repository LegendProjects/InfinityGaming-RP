...
function OpenCloakroomMenu()
	...
	if grade == 'ta_employee' then
		table.insert(elements, {label = _U('work_wear'), value = 'ta_employee'})
	elseif grade == 'ta_security' then
		table.insert(elements, {label = _U('work_wear'), value = 'ta_security'})
	elseif grade == 'ta_cashier' then
		table.insert(elements, {label = _U('work_wear'), value = 'ta_cashier'})
	elseif grade == 'ta_manager' then
		table.insert(elements, {label = _U('work_wear'), value = 'ta_manager'})
	elseif grade == 'ta_captain' then
		table.insert(elements, {label = _U('work_wear'), value = 'ta_captain'})
	elseif grade == 'ta_headsecurity' then
		table.insert(elements, {label = _U('work_wear'), value = 'ta_headsecurity'})
	elseif grade == 'ta_cfo' then
		table.insert(elements, {label = _U('work_wear'), value = 'ta_cfo'})
	elseif grade == 'boss' then
		table.insert(elements, {label = _U('work_wear'), value = 'boss'})
	end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom', {
		...
		if
			data.current.value == 'ta_employee' or
			data.current.value == 'ta_security' or
			data.current.value == 'ta_cashier' or
			data.current.value == 'ta_manager' or
			data.current.value == 'ta_captain' or
			data.current.value == 'ta_headsecurity' or
			data.current.value == 'ta_cfo' or
...
function StoreNearbyVehicle(playerCoords)
	...
	ESX.TriggerServerCallback('ig-bahamamamas:storeNearbyVehicle', function(storeSuccess, foundNum)
...
function GetAvailableVehicleSpawnPoint(station, part, partNum)
	local spawnPoints = Config.BahamaMamas[station][part][partNum].SpawnPoints
...
AddEventHandler('ig-bahamamamas:hasEnteredMarker', function(station, part, partNum)
...
AddEventHandler('ig-bahamamamas:hasExitedMarker', function(station, part, partNum)
...
Citizen.CreateThread(function()
	...
	for k,v in pairs(Config.BahamaMamas) do
		...
		AddTextComponentString('Entertainment: Bahama Mamas')
...
Citizen.CreateThread(function()
	while true do
		...
		if ESX.PlayerData.job and ESX.PlayerData.job.name == 'bahamamamas' then
			...
			if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastStation ~= currentStation or LastPart ~= currentPart or LastPartNum ~= currentPartNum)) then
				...
				then
					TriggerEvent('ig-bahamamamas:hasExitedMarker', LastStation, LastPart, LastPartNum)
					hasExited = true
				end
				...
				TriggerEvent('ig-bahamamamas:hasEnteredMarker', currentStation, currentPart, currentPartNum)
			end
	...
	while true do
		...
		if ESX.PlayerData.job and ESX.PlayerData.job.name == 'bahamamamas' then
			if CurrentAction then
				...
				if IsControlJustReleased(0, 38) and ESX.PlayerData.job and ESX.PlayerData.job.name == 'bahamamamas' then
					...
					elseif CurrentAction == 'menu_boss_actions' then
						ESX.UI.Menu.CloseAll()
						TriggerEvent('iG_society:openBossMenu', 'bahamamamas', function(data, menu)
							menu.close()
...
function OpenJobActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'job_actions_menu', {
		title    = "Job Actions Menu",
		align    = 'right',
		elements = {
		...
		if data.current.value == 'billing' then
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'billing', {
				title = "Enter amount to invoice closest player:"
			}, function(data, menu)
				local amount = tonumber(data.value)

				if amount == nil or amount < 0 then
					ESX.ShowNotification("The amount you have entered is invalid.")
				else
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						menu.close()
						TriggerServerEvent('iG_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_bahamamamas', "Bahama Mamas", amount)
						-- if Config.EnableJobLogs == true then
						-- 	TriggerServerEvent('igLogs:AddInLog', "casino", "sendBill", GetPlayerName(PlayerId()), GetPlayerName(closestPlayer), "The Diamond Casino", amount)
						-- end
					end
				end
			end, function(data, menu)
