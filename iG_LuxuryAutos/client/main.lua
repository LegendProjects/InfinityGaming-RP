...
Citizen.CreateThread(function()
	...
	print('[DEBUG] Luxury Autos vehicles loaded.')
end)
...
AddEventHandler('iG:playerLoaded', function(xPlayer)
	...
	if Config.EnablePlayerManagement then
		if ESX.PlayerData.job.name == 'luxuryautos' then
...
function ReturnVehicleProvider()
	ESX.TriggerServerCallback('vy_LuxuryAutos:getCommercialVehicles', function(vehicles)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'return_provider_menu', {
			...
			TriggerServerEvent('vy_LuxuryAutos:returnProvider', data.current.value)
...
function OpenShopMenu()
	if #Vehicles == 0 then
		print('[vy_LuxuryAutos] [^3ERROR^7] No vehicles found')
		return
	end
	...
	for i=1, #Vehicles, 1 do
		if IsModelInCdimage(GetHashKey(Vehicles[i].model)) then
			...
			else print (Vehicles[i].category) end
		else
			print(('[vy_LuxuryAutos] [^3ERROR^7] Vehicle "%s" does not exist'):format(Vehicles[i].model))
		end
	end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
			...
			if data2.current.value == 'yes' then
				if Config.EnablePlayerManagement then
					ESX.TriggerServerCallback('vy_LuxuryAutos:buyCarDealerVehicle', function(success)
						if success then
							IsInShopMenu = false
					...
					ESX.TriggerServerCallback('vy_LuxuryAutos:buyVehicle', function(success)
						if success then
							IsInShopMenu = false
							menu2.close()
							menu.close()
...
function OpenResellerMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'reseller', {
		...
		elements = {
			...
			{label = "View Available Vehicles",                    value = 'buy_vehicle'},
			{label = "Return Vehicle to Manufacturer",                value = 'return_provider'},
			{label = "<strong> - - - [ Display ] - - - </strong>"},
			{label = "Put Vehicle on Display",                    value = 'pop_vehicle'},
			{label = "Return Vehicle From Display",                  value = 'depop_vehicle'},
			{label = "<strong> - - - [ Billing ] - - - </strong>"},
			{label = "Create Invoice for Customer",                    value = 'create_bill'},
			{label = "<strong> - - - [ Transfer Vehicles ] - - - </strong>"},
			{label = "Transfer Vehicle to Customer",         value = 'set_vehicle_owner_sell'},
			{label = "Transfer Vehicle to Society", value = 'set_vehicle_owner_sell_society'},
	}}, function(data, menu)
				...
				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'set_vehicle_owner_sell_amount', {
					...
					if amount == nil then
						...
						if closestPlayer == -1 or closestDistance > 3.0 then
							...
							TriggerServerEvent('iG_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_luxuryautos', _U('car_dealer'), tonumber(data2.value))
						end
					end
				end, function(data2, menu2)
					menu2.close()
		...
		elseif action == 'set_vehicle_owner_sell' then
			if currentDisplayVehicle then
				...
				if closestPlayer ~= -1 and closestDistance < 3 then
					...
					TriggerServerEvent('vy_LuxuryAutos:setVehicleOwnedPlayerId', GetPlayerServerId(closestPlayer), vehicleProps, CurrentVehicleData.model, CurrentVehicleData.name)
					currentDisplayVehicle = nil
					TriggerServerEvent('gl_Locksystem:sv_giveKeys', GetPlayerServerId(closestPlayer), newPlate .. ' ')
				else
					ESX.ShowNotification(_U('no_players'))
				end
			else
		...
		elseif action == 'set_vehicle_owner_sell_society' then
			if currentDisplayVehicle then
				...
				if closestPlayer == -1 or closestDistance > 3.0 then
					...
					ESX.TriggerServerCallback('iG:getOtherPlayerData', function (xPlayer)
						...
						vehicleProps.plate = newPlate
						SetVehicleNumberPlateText(currentDisplayVehicle, newPlate)
						TriggerServerEvent('vy_LuxuryAutos:sellVehicle', model)
						TriggerServerEvent('vy_LuxuryAutos:addToList', GetPlayerServerId(closestPlayer), model, newPlate)
						TriggerServerEvent('vy_LuxuryAutos:setVehicleOwnedSociety', xPlayer.job.name, vehicleProps)
						-- TriggerServerEvent('igLogs:AddInLog', 'vehicleshop', 'carbuysociety', GetPlayerName(closestPlayer), GetPlayerName(PlayerId()), vehicleProps.plate, VehicleName)
...
function OpenRentedVehiclesMenu()
	ESX.TriggerServerCallback('vy_LuxuryAutos:getRentedVehicles', function(vehicles)
		local elements = {}
...
function OpenBossActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'reseller',{
		...
		if data.current.value == 'boss_actions' then
			TriggerEvent('iG_society:openBossMenu', 'luxuryautos', function(data2, menu2)
				menu2.close()
			end)
		elseif data.current.value == 'sold_vehicles' then

			ESX.TriggerServerCallback('vy_LuxuryAutos:getSoldVehicles', function(customers)
				local elements = {
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('vy_LuxuryAutos:getStockItems', function(items)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					TriggerServerEvent('vy_LuxuryAutos:getStockItem', itemName, count)
					menu2.close()
					menu.close()
					OpenGetStocksMenu()
				end
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('vy_LuxuryAutos:getPlayerInventory', function(inventory)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					TriggerServerEvent('vy_LuxuryAutos:putStockItems', itemName, count)
					menu2.close()
					menu.close()
					OpenPutStocksMenu()
				end
...
AddEventHandler('vy_LuxuryAutos:hasEnteredMarker', function(zone)
	if zone == 'ShopEntering' then

		if Config.EnablePlayerManagement then
			if ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'luxuryautos' then
				CurrentAction     = 'reseller_menu'
				CurrentActionMsg  = _U('shop_menu')
				CurrentActionData = {}
			end
	...
	elseif zone == 'BossActions' and Config.EnablePlayerManagement and ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'luxuryautos' and ESX.PlayerData.job.grade_name == 'boss' then
		CurrentAction     = 'boss_actions_menu'
		CurrentActionMsg  = _U('shop_menu')
		CurrentActionData = {}
	end
...
AddEventHandler('vy_LuxuryAutos:hasExitedMarker', function(zone)
...
Citizen.CreateThread(function()
	...
	AddTextComponentString('Dealership: Luxury Autos')
...
Citizen.CreateThread(function()
	while true do
		...
		if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
			...
			TriggerEvent('vy_LuxuryAutos:hasEnteredMarker', currentZone)
		end

		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('vy_LuxuryAutos:hasExitedMarker', LastZone)
		end
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) then
				if CurrentAction == 'shop_menu' then
					-- TriggerServerEvent('vy_CarShowroom:syncVehicles')
					exports['mythic_progbar']:Progress({
						name = "fetching_vehicles",
						duration = 1000,
						label = "Fetching Vehicles..",
						useWhileDead = false,
						canCancel = true,
						controlDisables = {
				...
				elseif CurrentAction == 'give_back_vehicle' then
					ESX.TriggerServerCallback('vy_LuxuryAutos:giveBackVehicle', function(isRentedVehicle)
				...
				elseif CurrentAction == 'resell_vehicle' then
					ESX.TriggerServerCallback('vy_LuxuryAutos:resellVehicle', function(vehicleSold)
						if vehicleSold then
							ESX.Game.DeleteVehicle(CurrentActionData.vehicle)
