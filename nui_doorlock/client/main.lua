...
Citizen.CreateThread(function()
	...
	exports['ig-keybinds']:RegisterKeybind('ToggleDoorLock', 'Toggle Nearby Door Lock', 'E', ToggleDoorLock)
	Citizen.Wait(1000)
...
-- AddEventHandler('iG:setJob2', function(job)
...
function IsAuthorized(doorID)
	...
	if doorID.authorizedJobs then
		for job,rank in pairs(doorID.authorizedJobs) do
			if (job == ESX.PlayerData.job.name and rank <= ESX.PlayerData.job.grade) or (ESX.PlayerData.org and job == ESX.PlayerData.org.name and rank <= ESX.PlayerData.org.grade) then
				canOpen = true
			end
		end
	end
...
ToggleDoorLock = function()
	if not isCuffed and closestDoor and IsAuthorized(closestV) then
...
RegisterNetEvent('ig-thieving:lockpickdoor') -- Modify for whichever lockpick you're using
AddEventHandler('ig-thieving:lockpickdoor', function()
	if not isCuffed and closestDoor and closestV.lockpick then
		TaskTurnPedToFaceCoord(playerPed, closestV.textCoords, 0)
		exports['mythic_progbar']:Progress({
			name = "lockpicking",
			duration = 18500,
			label = "Thieving: Lockpicking Door...",
			useWhileDead = false,
			canCancel = true,
			controlDisables = {
			...
			if not cancelled then
				...
				if chance >= 250 then
					...
					TriggerEvent('vy_Base:Add_SkillExp', 'thieving', 7)
				elseif chance <= 249 then
					exports['mythic_notify']:SendAlert('error', 'Thieving: Lockpicking unsuccessful.')
				end
			end
		end)
	end
