local Keys = {
	["ESC"] = 322, ["F1"] = 288, ["F2"] = 289, ["F3"] = 170, ["F5"] = 166, ["F6"] = 166, ["F7"] = 168, ["F8"] = 169, ["F9"] = 56, ["F10"] = 57,
...
function StopNPCJob(cancel)
	...
	if cancel then
		...
		TriggerServerEvent('iG_poolcleaner:GiveItem')
		StartNPCJob()
		Done = true
	end
	...
	while true do
		...
		if PlayerData.job2 ~= nil and PlayerData.job2.name == Config.nameJob then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('iG_poolcleaner:hasEnteredMarker', currentZone)
			end
			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('iG_poolcleaner:hasExitedMarker', LastZone)
			end
				...
				if (IsControlJustReleased(1, Keys["E"]) or IsControlJustReleased(2, Keys["RIGHT"])) and PlayerData.job2 ~= nil then
					...
					if PlayerData.job2.name == Config.nameJob then
						...
						if CurrentAction == 'Sale' then
							TriggerServerEvent('iG_poolcleaner:startSale')
						end
							...
							if string.find (plate, platePrefix) then
								...
								if hash == GetHashKey(truck.Hash) then
									if GetVehicleEngineHealth(vehicle) <= 500 or GetVehicleBodyHealth(vehicle) <= 500 then
										...
										TriggerServerEvent('iG_vehiclelock:vehjobSup', plate, 'no')
		...
		function(data, menu)
			for i=1, #elements, 1 do
				...
				ESX.Game.SpawnVehicle(data.current.value.Hash, coords, Heading, function(vehicle)
					...
					TriggerServerEvent('iG_vehiclelock:registerkeyjob', name, plate, 'no')
...
AddEventHandler('iG_poolcleaner:hasEnteredMarker', function(zone)
...
AddEventHandler('iG_poolcleaner:hasExitedMarker', function(zone)

	if zone == 'Sale' then
		TriggerServerEvent('iG_poolcleaner:stopSale')
	end
	CurrentAction = nil
	ESX.UI.Menu.CloseAll()
end)
