...
Citizen.CreateThread(function()
	...
	ESX.TriggerServerCallback('iG_boatshop:getCategories', function(categories)
		Categories = categories
	end)

	ESX.TriggerServerCallback('iG_boatshop:getVehicles', function(vehicles)
		Vehicles = vehicles
	end)
end)
...
RegisterNetEvent('iG_boatshop:sendCategories')
AddEventHandler('iG_boatshop:sendCategories', function(categories)
	Categories = categories
end)

RegisterNetEvent('iG_boatshop:sendVehicles')
AddEventHandler('iG_boatshop:sendVehicles', function(vehicles)
	Vehicles = vehicles
end)
...
function OpenBuyLicenseMenu(zone)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_license', {
		...
		if data.current.value == 'yes' then
			ESX.TriggerServerCallback('iG_boatshop:buyLicense', function(bought)
				if bought then
					menu.close()
					OpenShopMenu()
				end
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
			...
			if data2.current.value == 'yes' then
				local playerData = ESX.GetPlayerData()
					ESX.TriggerServerCallback('iG_boatshop:buyVehicle', function(hasEnoughMoney)
						if hasEnoughMoney then
							...
							ESX.Game.SpawnVehicle(vehicleData.model, Config.Zones.ShopOutside.Pos, Config.Zones.ShopOutside.Heading, function(vehicle)
								...
								if Config.EnableOwnedVehicles then
									TriggerServerEvent('iG_boatshop:setVehicleOwned', vehicleProps)
								end
...
-- Entered Marker
AddEventHandler('iG_boatshop:hasEnteredMarker', function (zone)
...
-- Exited Marker
AddEventHandler('iG_boatshop:hasExitedMarker', function(zone)
...
Citizen.CreateThread(function()
	while true do
		...
		if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
			...
			TriggerEvent('iG_boatshop:hasEnteredMarker', currentZone)
		end

		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('iG_boatshop:hasExitedMarker', LastZone)
		end
			...
			if IsControlJustReleased(0, Keys['E']) then
				if CurrentAction == 'shop_menu' then
					if Config.LicenseEnable then
						ESX.TriggerServerCallback('iG_license:checkLicense', function(hasBoatingLicense)
				...
				-- 	ESX.TriggerServerCallback('iG_boatshop:resellVehicle', function(vehicleSold)
				-- 		if vehicleSold then
