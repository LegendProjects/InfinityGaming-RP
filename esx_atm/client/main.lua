...
RegisterNetEvent('iG_atm:closeATM')
AddEventHandler('iG_atm:closeATM', function()
	SetNuiFocus(false)
	menuIsShowed = false
	SendNUIMessage({
		hideAll = true
...
RegisterNUICallback('escape', function(data, cb)
	TriggerEvent('iG_atm:closeATM')
	cb('ok')
end)

RegisterNUICallback('deposit', function(data, cb)
	TriggerServerEvent('iG_atm:deposit', data.amount)
	cb('ok')
end)

RegisterNUICallback('withdraw', function(data, cb)
	TriggerServerEvent('iG_atm:withdraw', data.amount)
	cb('ok')
end)
	...
	while true do
		...
		if isInATMMarker and not menuIsShowed then
			...
			if IsControlJustReleased(0, Keys['E']) and IsPedOnFoot(PlayerPedId()) then
				menuIsShowed = true
				ESX.TriggerServerCallback('iG:getPlayerData', function(data)
					SendNUIMessage({
						showMenu = true,
						player = {
							money = data.money,
