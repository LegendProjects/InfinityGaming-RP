...
Citizen.CreateThread(function()
	...
	ESX.TriggerServerCallback('iG_aircraftshop:getCategories', function(categories)
		Categories = categories
	end)

	ESX.TriggerServerCallback('iG_aircraftshop:getVehicles', function(vehicles)
		Vehicles = vehicles
	end)
end)
...
RegisterNetEvent('iG_aircraftshop:sendCategories')
AddEventHandler('iG_aircraftshop:sendCategories', function(categories)
	Categories = categories
end)

RegisterNetEvent('iG_aircraftshop:sendVehicles')
AddEventHandler('iG_aircraftshop:sendVehicles', function(vehicles)
	Vehicles = vehicles
end)
...
function OpenBuyLicenseMenu(zone)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_license', {
		...
		if data.current.value == 'yes' then
			ESX.TriggerServerCallback('iG_aircraftshop:buyLicense', function(bought)
				if bought then
					menu.close()
					OpenShopMenu()
				end
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
			...
			if data2.current.value == 'yes' then
				local playerData = ESX.GetPlayerData()
					ESX.TriggerServerCallback('iG_aircraftshop:buyVehicle', function(hasEnoughMoney)
						if hasEnoughMoney then
							...
							ESX.Game.SpawnVehicle(vehicleData.model, Config.Zones.ShopOutside.Pos, Config.Zones.ShopOutside.Heading, function(vehicle)
								...
								if Config.EnableOwnedVehicles then
									TriggerServerEvent('iG_aircraftshop:setVehicleOwned', vehicleProps)
								end
...
-- Entered Marker
AddEventHandler('iG_aircraftshop:hasEnteredMarker', function(zone)
...
-- Exited Marker
AddEventHandler('iG_aircraftshop:hasExitedMarker', function(zone)
...
Citizen.CreateThread(function()
	while true do
		...
		if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
			...
			TriggerEvent('iG_aircraftshop:hasEnteredMarker', currentZone)
		end

		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('iG_aircraftshop:hasExitedMarker', LastZone)
		end
			...
			if IsControlJustReleased(0, Keys['E']) then
				if CurrentAction == 'shop_menu' then
					if Config.LicenseEnable then
						ESX.TriggerServerCallback('iG_license:checkLicense', function(hasAircraftLicense)
				...
				-- 	ESX.TriggerServerCallback('iG_aircraftshop:resellVehicle', function(vehicleSold)
				-- 		if vehicleSold then
