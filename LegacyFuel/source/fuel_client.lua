...
AddEventHandler('iG_Fuel:startFuelUpTick', function(pumpObject, ped, vehicle)
	...
	if pumpObject then
		TriggerServerEvent('iG_Fuel:pay', currentCost)
	end

	currentCost = 0.0
end)

AddEventHandler('iG_Fuel:refuelFromPump', function(pumpObject, ped, vehicle)
	...
	TriggerEvent('iG_Fuel:startFuelUpTick', pumpObject, ped, vehicle)
						...
						if GetVehicleFuelLevel(vehicle) < 95 and canFuel then
							if currentCash > 0 then
								...
								if IsControlJustReleased(0, 38) then
									...
									TriggerEvent('iG_Fuel:refuelFromPump', isNearPump, ped, vehicle)
									LoadAnimDict("timetable@gardener@filling_can")
								end
							else
					...
					if currentCash >= Config.JerryCanCost then
						if not HasPedGotWeapon(ped, 883325847) then
							...
							if IsControlJustReleased(0, 38) then
								...
								TriggerServerEvent('iG_Fuel:pay', Config.JerryCanCost)
							...
							if Config.UseESX then
								...
								if refillCost > 0 then
									if currentCash >= refillCost then
										...
										if IsControlJustReleased(0, 38) then
											TriggerServerEvent('iG_Fuel:pay', refillCost)
