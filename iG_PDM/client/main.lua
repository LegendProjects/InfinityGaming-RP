...
function ReturnVehicleProvider()
	ESX.TriggerServerCallback('vy_PDM:getCommercialVehicles', function(vehicles)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'return_provider_menu', {
			...
			TriggerServerEvent('vy_PDM:returnProvider', data.current.value)
...
function OpenShopMenu()
	if #Vehicles == 0 then
		print('[vy_PDM] [^3ERROR^7] No vehicles found')
		return
	end
	...
	for i=1, #Vehicles, 1 do
		if IsModelInCdimage(GetHashKey(Vehicles[i].model)) then
			...
			print(('[vy_PDM] [^3ERROR^7] Vehicle "%s" does not exist'):format(Vehicles[i].model))
		end
	end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
			...
			if data2.current.value == 'yes' then
				if Config.EnablePlayerManagement then
					ESX.TriggerServerCallback('vy_PDM:buyCarDealerVehicle', function(success)
						if success then
							IsInShopMenu = false
					...
					ESX.TriggerServerCallback('vy_PDM:buyVehicle', function(success)
						if success then
							IsInShopMenu = false
							menu2.close()
							menu.close()
				...
				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'set_vehicle_owner_sell_amount', {
					...
					if amount == nil then
						...
						if closestPlayer == -1 or closestDistance > 3.0 then
							...
							TriggerServerEvent('iG_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_pdm', _U('car_dealer'), tonumber(data2.value))
						end
					end
				end, function(data2, menu2)
					menu2.close()
		...
		elseif action == 'set_vehicle_owner_sell' then
			if currentDisplayVehicle then
				...
				if closestPlayer ~= -1 and closestDistance < 3 then
					...
					TriggerServerEvent('vy_PDM:setVehicleOwnedPlayerId', GetPlayerServerId(closestPlayer), vehicleProps, CurrentVehicleData.model, CurrentVehicleData.name)
			...
			if currentDisplayVehicle then
				...
				if closestPlayer == -1 or closestDistance > 3.0 then
					...
					ESX.TriggerServerCallback('iG:getOtherPlayerData', function (xPlayer)
						...
						vehicleProps.plate = newPlate
						SetVehicleNumberPlateText(currentDisplayVehicle, newPlate)
						TriggerServerEvent('vy_PDM:sellVehicle', model)
						TriggerServerEvent('vy_PDM:addToList', GetPlayerServerId(closestPlayer), model, newPlate)
						TriggerServerEvent('vy_PDM:setVehicleOwnedSociety', xPlayer.job.name, vehicleProps)
...
function OpenRentedVehiclesMenu()
	ESX.TriggerServerCallback('vy_PDM:getRentedVehicles', function(vehicles)
		local elements = {}
...
function OpenBossActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'reseller',{
		...
		if data.current.value == 'boss_actions' then
			TriggerEvent('iG_society:openBossMenu', 'pdm', function(data2, menu2)
				menu2.close()
			end)
		elseif data.current.value == 'sold_vehicles' then

			ESX.TriggerServerCallback('vy_PDM:getSoldVehicles', function(customers)
				local elements = {
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('vy_PDM:getStockItems', function(items)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					TriggerServerEvent('vy_PDM:getStockItem', itemName, count)
					menu2.close()
					menu.close()
					OpenGetStocksMenu()
				end
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('vy_PDM:getPlayerInventory', function(inventory)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					TriggerServerEvent('vy_PDM:putStockItems', itemName, count)
					menu2.close()
					menu.close()
					OpenPutStocksMenu()
				end
...
AddEventHandler('vy_PDM:hasEnteredMarker', function(zone)
...
AddEventHandler('vy_PDM:hasExitedMarker', function(zone)
...
Citizen.CreateThread(function()
	...
	AddTextComponentString('Dealership: Premium Deluxe Motors')
	...
	while true do
		...
		if ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'pdm' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('vy_PDM:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('vy_PDM:hasExitedMarker', LastZone)
			end
			...
			if CurrentAction then
				...
				if IsControlJustReleased(0, 38) then
					...
					elseif CurrentAction == 'give_back_vehicle' then
						ESX.TriggerServerCallback('vy_PDM:giveBackVehicle', function(isRentedVehicle)
					...
					elseif CurrentAction == 'resell_vehicle' then
						ESX.TriggerServerCallback('vy_PDM:resellVehicle', function(vehicleSold)
							if vehicleSold then
								ESX.Game.DeleteVehicle(CurrentActionData.vehicle)
