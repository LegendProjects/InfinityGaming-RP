...
Citizen.CreateThread(function()
	...
	ESX.TriggerServerCallback('iG_license:checkLicense', function(boolean)
		if boolean then
			hasWeaponTraining = true
		else
			hasWeaponTraining = false
		end
	end, GetPlayerServerId(PlayerId()), 'weapontraining')
	print(hasWeaponTraining)
end)

RegisterNetEvent('vy_Weapons:UpdateLicenses')
AddEventHandler('vy_Weapons:UpdateLicenses', function() 
	Citizen.Wait(5000)
	ESX.TriggerServerCallback('iG_license:checkLicense', function(boolean)
		if boolean then
			hasWeaponTraining = true
...
-- These weapons will have semi-automatic and burst shot
Config.Weapons.Burst = {
	"WEAPON_APPISTOL",
}
		...
		if IsPedArmed(PlayerPed, 4) and not IsPedInAnyVehicle(PlayerPed, true) then
			...
			if PedWeapon ~= GetHashKey('WEAPON_STUNGUN') then
				if FireMode.LastWeapon ~= PedWeapon then
					...
					for _, Weapon in ipairs(Config.Weapons.Burst) do
						-- If weapon is in list
						if GetHashKey(Weapon) == PedWeapon then
							-- Set weapon type to semi-automatic
							Active = "burst"
			...
			if Active and Active ~= "reticle" then
					...
					if Active == "full" then
						if FireMode.Weapons[PedWeapon] <= 2 then
							if FireMode.Weapons[PedWeapon] == 0 then
								exports['mythic_notify']:SendAlert('inform', 'Weapon: Semi-Automatic.') 
							elseif FireMode.Weapons[PedWeapon] == 1 then
								exports['mythic_notify']:SendAlert('inform', 'Weapon: Burst.') 
							elseif FireMode.Weapons[PedWeapon] == 2 then
								exports['mythic_notify']:SendAlert('inform', 'Weapon: Fully Automatic.') 
							end
							PlaySoundFrontend(-1, "Faster_Click", "RESPAWN_ONLINE_SOUNDSET", 1)
							FireMode.Weapons[PedWeapon] = FireMode.Weapons[PedWeapon] + 1
						elseif FireMode.Weapons[PedWeapon] >= 3 then
							exports['mythic_notify']:SendAlert('inform', 'Weapon: Safety Enabled.')  
							PlaySoundFrontend(-1, "Reset_Prop_Position", "DLC_Dmod_Prop_Editor_Sounds", 0)
							FireMode.Weapons[PedWeapon] = 0
						end
					elseif Active == "burst" then 
				...
				elseif IsDisabledControlJustPressed(1, 24) then
					...
					if FireMode.Weapons[PedWeapon] == 0 then
						...
						exports['mythic_notify']:SendAlert('error', 'Weapon: Safety enabled, press \'C\' to disable!')
...
Citizen.CreateThread(function()
	while true do
		...
		if FireMode.LastWeaponActive ~= "reticle" then 
			if not hasWeaponTraining then 
