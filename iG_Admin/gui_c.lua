...
RegisterCommand('ig_admin', function(source, args)
	CreateThread(function()
		if not RedM then
			playerlist = nil
			TriggerServerEvent("iG_Admin:GetInfinityPlayerList") -- shitty fix for bigmode
			repeat
				Wait(100)
			until playerlist
		end
		...
		if strings then
			...
			TriggerServerEvent("iG_Admin:amiadmin")
		end
	end)
end)
...
Citizen.CreateThread(function()
	...
	TriggerServerEvent("iG_Admin:amiadmin")
	TriggerServerEvent("iG_Admin:requestBanlist")
	TriggerServerEvent("iG_Admin:requestCachedPlayers")
	...
	mainMenu = NativeUI.CreateMenu("iG_Admin", "~b~Admin Menu", menuOrientation, 0)
...
OpenAdminMenu = function ()
	if (not RedM and GetLastInputMethod( 0 )) then
...
function GenerateMenu() -- this is a big ass function
	...
	for i,thePlayer in pairs(players) do
		...
		if permissions["kick"] then
			...
			thisItem.Activated = function(ParentMenu,SelectedItem)
				...
				TriggerServerEvent("iG_Admin:kickPlayer", thePlayer.id, KickReason)
				BanTime = 1
				BanReason = ""
		...
		if permissions["ban"] then
			...
			thisItem.Activated = function(ParentMenu,SelectedItem)
				...
				TriggerServerEvent("iG_Admin:banPlayer", thePlayer.id, BanReason, banLength[BanTime].time, thePlayer.name )
				BanTime = 1
				BanReason = ""
				_menuPool:CloseAllMenus()
				Citizen.Wait(800)
		...
		if permissions["mute"] then			
			...
			thisItem.Activated = function(ParentMenu,SelectedItem)
				TriggerServerEvent("iG_Admin:mutePlayer", thePlayer.id)
			end
		end
		...
		if permissions["spectate"] then
			...
			thisPlayer:AddItem(thisItem)
			thisItem.Activated = function(ParentMenu,SelectedItem)
				TriggerServerEvent("iG_Admin:requestSpectate",thePlayer.id)
			end
		end
		...
		if permissions["teleport.player"] then
			...
			thisItem.Activated = function(ParentMenu,SelectedItem)
				if not RedM then
					TriggerServerEvent('iG_Admin:TeleportAdminToPlayer', thePlayer.id)
				else
		...
		if permissions["teleport.player"] then
			...
			thisItem.Activated = function(ParentMenu,SelectedItem)
				...
				TriggerServerEvent("iG_Admin:TeleportPlayerToCoords", thePlayer.id, coords)
			end
		end
		...
		if permissions["slap"] then
			...
			thisItem.OnSliderSelected = function(index)
				TriggerServerEvent("iG_Admin:SlapPlayer", thePlayer.id, index*10)
			end
		end
			...
			thisPlayer.OnListSelect = function(sender, item, index)
					if item == thisItem then
							...
							if i == GetLocalisedText("on") then
								TriggerServerEvent("iG_Admin:FreezePlayer", thePlayer.id, true)
							else
								TriggerServerEvent("iG_Admin:FreezePlayer", thePlayer.id, false)
							end
					end
			end
		end
		...
		if permissions["screenshot"] then
			...
			thisPlayer:AddItem(thisItem)
			thisItem.Activated = function(ParentMenu,SelectedItem)
				TriggerServerEvent("iG_Admin:TakeScreenshot", thePlayer.id)
			end
		end
	...
	if permissions["teleport.everyone"] then
		...
		thisItem.Activated = function(ParentMenu,SelectedItem)
			...
			TriggerServerEvent("iG_Admin:TeleportPlayerToCoords", -1, pCoords)
		end
	end
		...
		for i, cachedplayer in pairs(cachedplayers) do
			if cachedplayer.droppedTime and not cachedplayer.immune then
				...
				thisItem.Activated = function(ParentMenu,SelectedItem)
					...
					TriggerServerEvent("iG_Admin:offlinebanPlayer", cachedplayer.id, BanReason, banLength[BanTime].time, cachedplayer.name)
					BanTime = 1
					BanReason = ""
					_menuPool:CloseAllMenus()
					Citizen.Wait(800)
	...
	if permissions["manageserver"] then
		...
		thisItem.Activated = function(ParentMenu,SelectedItem)
			...
			if result then
				TriggerServerEvent("iG_Admin:SetGameType", result)
			end
		end
		...
		thisItem.Activated = function(ParentMenu,SelectedItem)
			...
			if result then
				TriggerServerEvent("iG_Admin:SetMapName", result)
			end
		end
		...
		thisItem.Activated = function(ParentMenu,SelectedItem)
			...
			if result then
				TriggerServerEvent("iG_Admin:StartResource", result)
			end
		end
		...
		thisItem.Activated = function(ParentMenu,SelectedItem)
			...
			if result then
				if result ~= GetCurrentResourceName() and result ~= "NativeUI" then
					TriggerServerEvent("iG_Admin:StopResource", result)
				else
					TriggerEvent("chat:addMessage", { args = { "iG_Admin", GetLocalisedText("badidea") } })
				end
			end
		end
		...
		for i,theBanned in ipairs(banlist) do
			if i<(banlistPage*10)+1 and i>(banlistPage*10)-10 then
				if theBanned then
					...
					thisItem.Activated = function(ParentMenu,SelectedItem)
						TriggerServerEvent("iG_Admin:unbanPlayer", theBanned.banid)
						TriggerServerEvent("iG_Admin:requestBanlist")
						_menuPool:CloseAllMenus()
						Citizen.Wait(800)
						GenerateMenu()
	...
	if permissions["unban"] then
		...
		settingsMenu:AddItem(thisItem)
		thisItem.Activated = function(ParentMenu,SelectedItem)
			TriggerServerEvent("iG_Admin:updateBanlist")
		end
	end
	...
	if permissions["anon"] then
		...
		settingsMenu.OnCheckboxChange = function(sender, item, checked_)
			if item == thisItem then
				anonymous = checked_
				TriggerServerEvent("iG_Admin:SetAnonymous", checked_)
			end
		end
	end
	_menuPool:ControlDisablingEnabled(false)
	...
	while true do
		...
		if drawInfo then
			...
			if (not RedM and IsControlJustPressed(0,103) or (RedM and IsControlJustReleased(0, Controls["VehExit"]))) then
				...
				TriggerEvent('iG_Admin:FreezePlayer', false)
