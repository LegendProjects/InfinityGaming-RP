...
function StartTheoryTest()
	...
	-- TriggerServerEvent('iG_dmvschool:pay', Config.Prices['dmv'])
end
...
function StopTheoryTest(success)
	...
	if success then
		TriggerServerEvent('iG_dmvschool:addLicense', 'dmv')
...
function StartDriveTest(type)
	...
	-- TriggerServerEvent('iG_dmvschool:pay', Config.Prices[type])
end

function StopDriveTest(success)
	if success then
		TriggerServerEvent('iG_dmvschool:addLicense', CurrentTestType)
		ESX.ShowNotification(_U('passed_test'))
	else
		ESX.ShowNotification(_U('failed_test'))
	end
...
function OpenDMVSchoolMenu()
	...
	if ownedLicenses['dmv'] then
		if not ownedLicenses['drive'] then
			table.insert(elements, {
				label = (('%s'):format(_U('road_test_car'))),
				value = 'drive_test',
				type = 'drive'
			})
		end
		...
		if not ownedLicenses['drive_bike'] then
			table.insert(elements, {
				label = (('%s'):format(_U('road_test_bike'))),
				value = 'drive_test',
				type = 'drive_bike'
			})
		end
		...
		if not ownedLicenses['drive_truck'] then
			table.insert(elements, {
				label = (('%s'):format(_U('road_test_truck'))),
				value = 'drive_test',
				type = 'drive_truck'
			})
		end
...
AddEventHandler('iG_dmvschool:hasEnteredMarker', function(zone)
...
AddEventHandler('iG_dmvschool:hasExitedMarker', function(zone)
	CurrentAction = nil
	ESX.UI.Menu.CloseAll()
end)

RegisterNetEvent('iG_dmvschool:loadLicenses')
AddEventHandler('iG_dmvschool:loadLicenses', function(licenses)
	Licenses = licenses
end)
...
Citizen.CreateThread(function()
	...
	AddTextComponentString('Government: Department of Transport, NSW')
...
Citizen.CreateThread(function()
	while true do
		...
		if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
			...
			TriggerEvent('iG_dmvschool:hasEnteredMarker', currentZone)
		end

		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('iG_dmvschool:hasExitedMarker', LastZone)
		end
	end
end)
