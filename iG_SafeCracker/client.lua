...
RegisterNetEvent('iG_SafeCracker:StartMinigame')
AddEventHandler('iG_SafeCracker:StartMinigame', function(rewards,stayClosed) MFSC:StartMinigame(rewards,stayClosed); end)
...
function MFSC:InputCheck()
	...
	if 		IsControlPressed( 0, Keys[ 'G' ] ) 	or IsControlPressed( 0, Keys[ 'ESC' ] )		then self:EndMinigame(false); end
...
function MFSC:EndMinigame(won, rewards)
	...
	if won then 
		...
		msg = "Heist: You've successfully cracked the safe."
    if not self.StayClosed then
		...
		TriggerServerEvent('iG_SafeCracker:AddReward', rewards)
		
	else	
		PlaySoundFrontend(self.SoundID, self.Config.SafeResetSound, self.Config.SafeSoundset, true)
		msg = "Heist: You've failed to crack the safe."
	end

	TriggerEvent('iG:showNotification', msg)
  TriggerEvent('iG_SafeCracker:EndMinigame', won)
end

RegisterNetEvent('iG_SafeCracker:EndGame')
AddEventHandler('iG_SafeCracker:EndGame', function() MFSC:EndMinigame(); end)
...
function MFSC:Awake(...)
    ...
    ESX.TriggerServerCallback('iG_SafeCracker:GetStartData', function(retVal) self.dS = true; self.cS = retVal; self:Start(); end)
end
...
RegisterNetEvent('iG_SafeCracker:SpawnSafe')
AddEventHandler('iG_SafeCracker:SpawnSafe', function(tab, pos, heading, cb) if cb then cb(MFSC:SpawnSafeObject(tab,pos,heading)) else MFSC:SpawnSafeObject(tab,pos,heading); end; end)
