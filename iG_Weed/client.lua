...
for i,entry in ipairs(registerStrings) do
    AddTextEntry('Weed_'..entry, _U(entry))
end

function interactHelp(stage,action)
    BeginTextCommandDisplayHelp('Weed_status_active')
...
function passiveHelp(stage,status)
    BeginTextCommandDisplayHelp('Weed_status_passive')
    AddTextComponentInteger(stage)
    AddTextComponentInteger(#Growth)
...
function serverlog(...)
    TriggerServerEvent('iG_Weed:log',...)
end

RegisterNetEvent('iG_Weed:make_toast')
AddEventHandler ('iG_Weed:make_toast', function(subject,message)
...
RegisterNetEvent('iG_Weed:do')
AddEventHandler ('iG_Weed:do', function(scenarioName, location)
...
RegisterNetEvent('iG_Weed:attempt_plant')
AddEventHandler ('iG_Weed:attempt_plant', function()
    local plantable, message, location, _, soil = getPlantingLocation()
    if plantable then
        TriggerServerEvent('iG_Weed:success_plant', location, soil)
                ...
                if closestDistance <= Config.Distance.Interact then
                    ...
                    if now >= lastAction + Config.ActionTime then
                        if IsDisabledControlJustPressed(0, 44) then
                            ...
                            TriggerServerEvent('iG_Weed:remove', closestPlant.id, myLocation)
                        else
                            if stage.interact then
                                interactHelp(closestPlant.stage, _U(stage.label))
                                if IsControlJustPressed(0, 38) then
                                    lastAction = now
                                    TriggerServerEvent('iG_Weed:frob', closestPlant.id, myLocation)
                                end
                            else
...
RegisterNetEvent('iG_Weed:toggle_debug')
AddEventHandler ('iG_Weed:toggle_debug', function()
...
RegisterNetEvent('iG_Weed:pyromaniac')
AddEventHandler ('iG_Weed:pyromaniac',function(location)
...
RegisterNetEvent('iG_Weed:groundmat')
AddEventHandler ('iG_Weed:groundmat', function()
    local plantable, message, where, normal, material = getPlantingLocation(true)
...
RegisterNetEvent('iG_Weed:test_forest')
AddEventHandler ('iG_Weed:test_forest',function(count, randomStage)
    ...
    TriggerServerEvent('iG_Weed:test_forest', forest)
end)
