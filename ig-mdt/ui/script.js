    ...
    methods: {
        ...
        OffenderSearch() {
            if (this.offender_search) {
                ...
                $.post('http://ig-mdt/performOffenderSearch', JSON.stringify({
        ...
        OpenOffenderDetails(id) {
            for (var key in this.offender_results.results) {
                if (id == this.offender_results.results[key].id) {
                    ...
                    $.post('http://ig-mdt/viewOffender', JSON.stringify({
        ...
        SaveOffenderChanges() {
            $.post('http://ig-mdt/saveOffenderChanges', JSON.stringify({
        ...
        ReportSearch() {
            if (this.report_search) {
                ...
                $.post('http://ig-mdt/performReportSearch', JSON.stringify({
        ...
        SubmitNewReport() {
            if (this.report_new.title && this.report_new.char_id && (Object.keys(this.report_new.charges).length > 0) && this.report_new.incident) {
                $.post('http://ig-mdt/submitNewReport', JSON.stringify({
        ...
        OpenOffenderDetailsById(id) {
            $.post('http://ig-mdt/getOffender', JSON.stringify({
                char_id: id
            }));
        ...
        DeleteSelectedReport() {
            $.post('http://ig-mdt/deleteReport', JSON.stringify({
        ...
        SaveReportEditChanges() {
            $.post('http://ig-mdt/saveReportChanges', JSON.stringify({
                id: this.report_selected.id,
        ...
        VehicleSearch() {
            if (this.vehicle_search) {
                ...
                $.post('http://ig-mdt/vehicleSearch', JSON.stringify({
        ...
        OpenVehicleDetails(result) {
            $.post('http://ig-mdt/getVehicle', JSON.stringify({
                vehicle: result
            }));
        ...
        SubmitNewWarrant() {
            ...
            $.post('http://ig-mdt/submitNewWarrant', JSON.stringify({
        ...
        DeleteSelectedWarrant() {
            $.post('http://ig-mdt/deleteWarrant', JSON.stringify({
        ...
        OpenReportById(id) {
            $.post('http://ig-mdt/getReport', JSON.stringify({
                id: id
            }));
            this.modal = 'loading';
            return;
            ...
            for (var key in this.report_selected.charges) {
                for (var offense in this.offenses) {
                    if (this.offenses[offense].label == key) {
                        ...
                        jailtime = this.report_selected.sentence
                    }
                }
            }
            $.post('http://ig-mdt/sentencePlayer', JSON.stringify({
        ...
        AttachToCall(index) {
            $.post('http://ig-mdt/attachToCall', JSON.stringify({
                index: index,
        ...
        DetachFromCall(index) {
            $.post('http://ig-mdt/detachFromCall', JSON.stringify({
                index: index
            }));
            this.current_call = {
                source: null,
        ...
        SetCallWaypoint(index) {
            $.post('http://ig-mdt/setCallWaypoint', JSON.stringify({
                coords: this.calls[index].coords
            }));
            return;
        },
        ...
        SaveEditCall() {
            $.post('http://ig-mdt/editCall', JSON.stringify({
        ...
        DeleteCall(index) {
            $.post('http://ig-mdt/deleteCall', JSON.stringify({
                index: index
            }));
            return;
        },
    ...
    computed: {
        ...
        filtered_bolos() {
            return this.bolos.filter(bolo => {
                if (
                    bolo.plate.toLowerCase().search(this.bolo_search.toLowerCase()) != -1
                )
                    return bolo;
            })
        }
        ...
        window.addEventListener('message', function(event) {
            if (event.data.type == "enable") {
                ...
                if (mdtApp.current_call.id == mdtApp.calls[event.data.call].id) {
                    $.post('http://ig-mdt/deleteCallBlip')
                    mdtApp.current_call = {
                        source: null,
                        details: null,
                        id: null,
