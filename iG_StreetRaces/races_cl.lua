        ...
        if IsPedInAnyVehicle(player, false) then
            ...
            if raceStatus.state == RACE_STATE_RACING then
                ...
                if raceStatus.checkpoint == 0 then
                    ...
                    if config_cl.checkpointRadius > 0 then
                        ...
                        checkpoint.checkpoint = CreateCheckpoint(checkpointType, checkpoint.coords.x,  checkpoint.coords.y, checkpoint.coords.z, 0, 0, 0, config_cl.checkpointRadius, 66, 158, 245, 127, 0)
                    ...
                    if GetDistanceBetweenCoords(position.x, position.y, position.z, checkpoint.coords.x, checkpoint.coords.y, 0, false) < config_cl.checkpointProximity then
                        ...
                        if raceStatus.checkpoint == #(race.checkpoints) then
                            ...
                            if config_cl.checkpointRadius > 0 then
                                local checkpointType = raceStatus.checkpoint < #race.checkpoints and RACE_CHECKPOINT_TYPE or RACE_CHECKPOINT_FINISH_TYPE
                                nextCheckpoint.checkpoint = CreateCheckpoint(checkpointType, nextCheckpoint.coords.x,  nextCheckpoint.coords.y, nextCheckpoint.coords.z, 0, 0, 0, config_cl.checkpointRadius, 66, 158, 245, 127, 0)
                ...
                if config_cl.hudEnabled then
                    ...
                    Draw2DText(config_cl.hudPosition.x, config_cl.hudPosition.y, ("~b~%02d:%06.3f"):format(timeMinutes, timeSeconds), 0.7)
                    local checkpoint = race.checkpoints[raceStatus.checkpoint]
                    ...
                    Draw2DText(config_cl.hudPosition.x, config_cl.hudPosition.y + 0.04, ("~b~CHECKPOINT %d/%d (%dm)"):format(raceStatus.checkpoint, #race.checkpoints, checkpointDist), 0.5)
                end
            ...
            elseif raceStatus.state == RACE_STATE_JOINED then
                ...
                elseif count <= config_cl.freezeDuration then
                    ...
                    Draw2DText(0.5, 0.4, ("~b~%d"):format(math.ceil(count/1000.0)), 3.0)
                    ...
                    Draw3DText(race.startCoords.x, race.startCoords.y, zCoord+1.0, ("Race for ~b~$%d~w~ starting in ~b~%d~w~s"):format(race.amount, math.ceil(count/1000.0)))
            ...
            -- Player is not in a race
                ...
                for index, race in pairs(races) do
                    ...
                    if proximity < config_cl.joinProximity and currentTime < race.startTime then
                        ...
                        Draw3DText(race.startCoords.x, race.startCoords.y, zCoord+1.0, ("Race for ~b~$%d~w~ starting in ~b~%d~w~s"):format(race.amount, count))
                        Draw3DText(race.startCoords.x, race.startCoords.y, zCoord+0.80, "Press [~b~E~w~] to join")
