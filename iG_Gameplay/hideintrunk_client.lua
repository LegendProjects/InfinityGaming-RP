    ...
    while true do
        ...
        if inTrunk then
            ...
            if DoesEntityExist(vehicle) or not IsPedDeadOrDying(PlayerPedId()) or not IsPedFatallyInjured(PlayerPedId()) then
                ...
                DrawText3D(coords, '[~b~E~w~] Leave trunk')
...
Citizen.CreateThread(function()
	while ESX == nil do TriggerEvent('iG:getSharedObject', function(obj) ESX = obj end) Wait(0) end
        ...
        if DoesEntityExist(vehicle) then
            ...
            if trunk ~= -1 then
                ...
                if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), coords, true) <= 1.5 then
                    if not inTrunk then
                        if GetVehicleDoorAngleRatio(vehicle, 5) < 0.9 then
                            DrawText3D(coords, '[~b~K~w~] Hide in trunk\n[~b~H~w~] Open trunk')
