...
RegisterNetEvent('iG_Tackle:getTackled')
AddEventHandler('iG_Tackle:getTackled', function(target)
...
RegisterNetEvent('iG_Tackle:playTackle')
AddEventHandler('iG_Tackle:playTackle', function()
	local playerPed = GetPlayerPed(-1)
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlPressed(0, Keys['LEFTSHIFT']) and IsControlPressed(0, Keys['G']) and IsControlPressed(0, Keys['W']) and not isTackling and GetGameTimer() - lastTackleTime > 20 * 1000 then
			Citizen.Wait(10)
			local closestPlayer, distance = ESX.Game.GetClosestPlayer()

			if distance ~= -1 and distance <= 2.5 and not isTackling and not isGettingTackled and not IsPedInAnyVehicle(GetPlayerPed(-1)) and not IsPedInAnyVehicle(GetPlayerPed(closestPlayer)) then
				isTackling = true
				lastTackleTime = GetGameTimer()

				TriggerServerEvent('iG_Tackle:tryTackle', GetPlayerServerId(closestPlayer))
			end
		end
	end
end)