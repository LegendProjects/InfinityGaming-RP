...
Config = {} 
Config.DamageNeeded = 150.0 -- 100.0 being broken and 1000.0 being fixed a lower value than 100.0 will break it
    ...
    while true do 
        ...
        if Vehicle.Vehicle ~= nil then
                ...
                if IsVehicleSeatFree(Vehicle.Vehicle, -1) and GetVehicleEngineHealth(Vehicle.Vehicle) <= Config.DamageNeeded then
                    ESX.Game.Utils.DrawText3D({x = Vehicle.Coords.x, y = Vehicle.Coords.y, z = Vehicle.Coords.z}, 'Press [~b~SHIFT~w~] and [~b~E~w~] to push the vehicle', 0.4)
                end
