...
function LoadDefaultModel(malePed, cb)
	...
	Citizen.CreateThread(function()
		...
		TriggerEvent('iG_skinchanger:modelLoaded')
	end)
end
...
AddEventHandler('iG_skinchanger:loadDefaultModel', function(loadMale, cb)
	LoadDefaultModel(loadMale, cb)
end)

AddEventHandler('iG_skinchanger:getData', function(cb)
...
AddEventHandler('iG_skinchanger:change', function(key, val)
	Character[key] = val

	if key == 'sex' then
		TriggerEvent('iG_skinchanger:loadSkin', Character)
	else
		ApplySkin(Character)
	end
end)

AddEventHandler('iG_skinchanger:getSkin', function(cb)
	cb(Character)
end)
...
RegisterNetEvent('iG_skinchanger:loadPedSkin')
AddEventHandler('iG_skinchanger:loadPedSkin', function(ped, skin)
	ApplySkin(skin, nil, ped)
end)

RegisterNetEvent('iG_skinchanger:loadSkin')
AddEventHandler('iG_skinchanger:loadSkin', function(skin, cb)
	...
	if skin['sex'] ~= LastSex then
		...
		if skin['sex'] == 0 then
			TriggerEvent('iG_skinchanger:loadDefaultModel', true, cb)
		elseif skin['sex'] > 1 then
			characterModel = pedList[skin.sex - 1]
		else
			TriggerEvent('iG_skinchanger:loadDefaultModel', false, cb)
		end
		RequestModel(characterModel)
	else
		ApplySkin(skin)
...
RegisterNetEvent('iG_skinchanger:loadClothes')
AddEventHandler('iG_skinchanger:loadClothes', function(playerSkin, clothesSkin)
	local playerPed = GetPlayerPed(-1)
	local characterModel
	if playerSkin['sex'] ~= LastSex then
		...
		if playerSkin['sex'] == 0 then
			TriggerEvent('iG_skinchanger:loadDefaultModel', true)
		elseif playerSkin['sex'] > 1 then
			characterModel = pedList[playerSkin.sex - 1]
		else
			TriggerEvent('iG_skinchanger:loadDefaultModel', false)
		end
		RequestModel(characterModel)
	else
...
RegisterNetEvent('iG_skinchanger:loadSkin2')
AddEventHandler('iG_skinchanger:loadSkin2', function(multipedID, skin)
	local MultiPed = {}
	for i=1, #Components, 1 do
