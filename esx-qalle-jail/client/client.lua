...
AddEventHandler("iG:playerLoaded", function(newData)
	PlayerData = newData

	Citizen.Wait(25000)

	ESX.TriggerServerCallback("iG_pol_jail:retrieveJailTime", function(inJail, newJailTime)
		if inJail then
...
AddEventHandler("iG:setJob", function(response)
	PlayerData["job"] = response
end)

RegisterNetEvent("iG_pol_jail:openJailMenu")
AddEventHandler("iG_pol_jail:openJailMenu", function()
	OpenJailMenu()
end)

RegisterNetEvent("iG_pol_jail:jailPlayer")
AddEventHandler("iG_pol_jail:jailPlayer", function(newJailTime)
	jailTime = newJailTime

	Cutscene()
end)

RegisterNetEvent("iG_pol_jail:unJailPlayer")
AddEventHandler("iG_pol_jail:unJailPlayer", function()
	jailTime = 0
...
function JailLogin()
	...
	exports['mythic_notify']:SendAlert('inform', 'You last logged off from prison, therefore you have been returned..')
...
function UnJail()
	...
	ESX.Game.Teleport(PlayerPedId(), vector3(1846.18, 2585.83, 45.88))

	ESX.TriggerServerCallback('iG_skin:getPlayerSkin', function(skin)
		TriggerEvent('iG_skinchanger:loadSkin', skin)
	end)
	
	exports['mythic_notify']:SendAlert('inform', 'You have been released from prison, enjoy!')
...
function InJail()
	...
	Citizen.CreateThread(function()
		...
		while jailTime > 0 do
			...
			exports['mythic_notify']:SendAlert('inform', 'You have ' .. jailTime .. ' minutes remaining in prison..')
			TriggerServerEvent("iG_pol_jail:updateJailTime", jailTime)

			if jailTime == 0 then
				UnJail()

				TriggerServerEvent("iG_pol_jail:updateJailTime", 0)
			end
		...
		while jailTime > 0 do
			...
			for posId, v in pairs(Packages) do
				...
				if DistanceCheck <= 10.0 then
					...
					ESX.Game.Utils.DrawText3D(v, "[~b~E~w~] " .. PackageText, 0.4)
		...
		if not IsEntityAttachedToEntity(packageId, PlayerPedId()) then
			...
			if DistanceCheck <= 2.0 then
				if IsControlJustPressed(0, 38) then
					...
					TriggerServerEvent("iG_pol_jail:prisonWorkReward")
				end
			end
		end
...
function OpenJailMenu()
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'jail_prison_menu',
		{
			...
			align    = 'right',
			elements = {
				{ label = "Jail (Closest Player)", value = "jail_closest_player" },
				{ label = "Release prisoner", value = "unjail_player" }
			}
		}, 
	function(data, menu)
			...
			ESX.UI.Menu.Open(
          	...
          	function(data2, menu2)
            	...
            	if jailTime == nil then
					exports['mythic_notify']:SendAlert('inform', 'The time needs to be in minutes.')
            	else
              		menu2.close()
              		...
              		if closestPlayer == -1 or closestDistance > 3.0 then
						exports['mythic_notify']:SendAlert('inform', 'No players are nearby.')
					else
						ESX.UI.Menu.Open(
						...
						function(data3, menu3)
						  	...
						  	if reason == nil then
								exports['mythic_notify']:SendAlert('inform', 'You failed to enter a reason.')
							else
								menu3.close()
								...
								if closestPlayer == -1 or closestDistance > 3.0 then
									exports['mythic_notify']:SendAlert('inform', 'No players are nearby.')
								  	...
								  	TriggerServerEvent("iG_pol_jail:jailPlayer", GetPlayerServerId(closestPlayer), jailTime, reason)
								end
		...
		elseif action == "unjail_player" then
			...
			ESX.TriggerServerCallback("iG_pol_jail:retrieveJailedPlayers", function(playerArray)
				...
				if #playerArray == 0 then
					exports['mythic_notify']:SendAlert('inform', 'There are no players currently in prison..')
					return
				end
