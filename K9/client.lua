...
--[[ NUI Callbacks ]]--
    ...
    RegisterNUICallback("spawnk9", function(data)
        TriggerEvent("iG_K9:ToggleK9", data.model)
    end)

    RegisterNUICallback("vehicletoggle", function(data)
        if spawned_ped ~= nil then
            TriggerServerEvent("iG_K9:RequestVehicleToggle")
        end
    end)

    RegisterNUICallback("vehiclesearch", function(data)
        if spawned_ped ~= nil then
            TriggerServerEvent("iG_K9:RequestItems")
        end
    end)
...
--[[ Main Event Handlers ]]--
    ...
    RegisterNetEvent("iG_K9:UpdateLanguage")
    AddEventHandler("iG_K9:UpdateLanguage", function(commands)
        language = commands
        Citizen.Trace(tostring(json.encode(language)))
    end)

    -- Opens K9 Menu
    RegisterNetEvent("iG_K9:OpenMenu")
    AddEventHandler("iG_K9:OpenMenu", function(pedRestriction, pedList)
    ...
    RegisterNetEvent("iG_K9:IdentifierRestricted")
    AddEventHandler("iG_K9:IdentifierRestricted", function()
        Notification(tostring("~r~You do not match any identifiers in the whitelist."))
    end)

    -- Spawns and Deletes K9
    RegisterNetEvent("iG_K9:ToggleK9")
    AddEventHandler("iG_K9:ToggleK9", function(model)
        if spawned_ped == nil then
            ...
            AddTextComponentString(tostring("iG_K9: ".. k9_name))
    ...
    RegisterNetEvent("iG_K9:ToggleFollow")
    AddEventHandler("iG_K9:ToggleFollow", function()
        if spawned_ped ~= nil then
    ...
    RegisterNetEvent("iG_K9:ToggleVehicle")
    AddEventHandler("iG_K9:ToggleVehicle", function(isRestricted, vehList)
    ...
    RegisterNetEvent("iG_K9:ToggleAttack")
    AddEventHandler("iG_K9:ToggleAttack", function(target)
    ...
    RegisterNetEvent("iG_K9:SearchVehicle")
    AddEventHandler("iG_K9:SearchVehicle", function(items, openDoors)
        local vehicle = GetVehicleAheadOfPlayer()
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            if IsControlPressed(1, 19) and IsControlJustPressed(1, 213) then
                TriggerServerEvent("iG_K9:RequestOpenMenu")
            end

            -- Trigger Attack
            if IsControlJustPressed(1, 100) and IsPlayerFreeAiming(PlayerId()) then
                local bool, target = GetEntityPlayerIsFreeAimingAt(PlayerId())

                if bool then
                    if IsEntityAPed(target) then
                        TriggerEvent("iG_K9:ToggleAttack", target)
                    end
                end
            end

            -- Trigger Follow
            if IsControlJustPressed(1, 100) and not IsPlayerFreeAiming(PlayerId()) then
                TriggerEvent("iG_K9:ToggleFollow")
            end
