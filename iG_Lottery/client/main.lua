...
function StoreNearbyVehicle(playerCoords)
	...
	ESX.TriggerServerCallback('iG_Lottery:storeNearbyVehicle', function(storeSuccess, foundNum)
...
function GetAvailableVehicleSpawnPoint(station, part, partNum)
	local spawnPoints = Config.Lottery[station][part][partNum].SpawnPoints
...
AddEventHandler('iG_Lottery:hasEnteredMarker', function(station, part, partNum)
...
AddEventHandler('iG_Lottery:hasExitedMarker', function(station, part, partNum)
...
Citizen.CreateThread(function()
	...
	for k,v in pairs(Config.Lottery) do
		...
		AddTextComponentString('Entertainment: Lottery Co.')
...
Citizen.CreateThread(function()
	while true do
		...
		if ESX.PlayerData.job and ESX.PlayerData.job.name == 'lottery' then
			...
			if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastStation ~= currentStation or LastPart ~= currentPart or LastPartNum ~= currentPartNum)) then
				...
				then
					TriggerEvent('iG_Lottery:hasExitedMarker', LastStation, LastPart, LastPartNum)
					hasExited = true
				end
				...
				TriggerEvent('iG_Lottery:hasEnteredMarker', currentStation, currentPart, currentPartNum)
			end
	...
	while true do
		...
		if ESX.PlayerData.job and ESX.PlayerData.job.name == 'lottery' then
			if CurrentAction then
				...
				if IsControlJustReleased(0, 38) and ESX.PlayerData.job and ESX.PlayerData.job.name == 'lottery' then
					...
					elseif CurrentAction == 'menu_boss_actions' then
						ESX.UI.Menu.CloseAll()
						TriggerEvent('iG_society:openBossMenu', 'lottery', function(data, menu)
							menu.close()
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'billing', {
				...
				if amount == nil or amount < 0 then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						menu.close()
						TriggerServerEvent('iG_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_lottery', "Lottery Co.", amount)
