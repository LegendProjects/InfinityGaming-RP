...
ESX.TriggerServerCallback = function(name, cb, ...)
	...
	TriggerServerEvent('iG:triggerServerCallback', name, ESX.CurrentRequestId, ...)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'inventory_item', {
			...
			if data1.current.action == 'give' then
				...
				if #playersNearby > 0 then
					...
					ESX.TriggerServerCallback('iG:getPlayerNames', function(returnedPlayers)
							...
							if playersNearby[selectedPlayer] then
								...
								if IsPedOnFoot(selectedPlayerPed) then
									if type == 'item_weapon' then
										TriggerServerEvent('iG:giveInventoryItem', selectedPlayerId, type, item, nil)
										menu2.close()
										menu1.close()
									else
										ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'inventory_item_count_give', {
											...
											if quantity then
												TriggerServerEvent('iG:giveInventoryItem', selectedPlayerId, type, item, quantity)
												menu3.close()
												menu2.close()
												menu1.close()
											else
			...
			elseif data1.current.action == 'remove' then
				if IsPedOnFoot(playerPed) then
					...
					if type == 'item_weapon' then
						...
						Citizen.Wait(1000)
						TriggerServerEvent('iG:removeInventoryItem', type, item)
					else
						ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'inventory_item_count_remove', {
							...
							if quantity then
								...
								TaskPlayAnim(playerPed, dict, anim, 8.0, 1.0, 1000, 16, 0.0, false, false, false)
								Citizen.Wait(1000)
								TriggerServerEvent('iG:removeInventoryItem', type, item, quantity)
							else
								ESX.ShowNotification(_U('amount_invalid'))
							end
						end, function(data2, menu2)
			...
			elseif data1.current.action == 'use' then
				TriggerServerEvent('iG:useItem', item)
							...
							ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'inventory_item_count_give', {
								...
								if quantity then
									if pedAmmo >= quantity and quantity > 0 then
										TriggerServerEvent('iG:giveInventoryItem', GetPlayerServerId(closestPlayer), 'item_ammo', item, quantity)
										menu2.close()
										menu1.close()
									else
...
RegisterNetEvent('iG:serverCallback')
AddEventHandler('iG:serverCallback', function(requestId, ...)
	ESX.ServerCallbacks[requestId](...)
	ESX.ServerCallbacks[requestId] = nil
end)

RegisterNetEvent('iG:showNotification')
AddEventHandler('iG:showNotification', function(msg, flash, saveToBrief, hudColorIndex)
	ESX.ShowNotification(msg, flash, saveToBrief, hudColorIndex)
end)

RegisterNetEvent('iG:showAdvancedNotification')
AddEventHandler('iG:showAdvancedNotification', function(sender, subject, msg, textureDict, iconType, flash, saveToBrief, hudColorIndex)
	ESX.ShowAdvancedNotification(sender, subject, msg, textureDict, iconType, flash, saveToBrief, hudColorIndex)
end)

RegisterNetEvent('iG:showHelpNotification')
AddEventHandler('iG:showHelpNotification', function(msg, thisFrame, beep, duration)
