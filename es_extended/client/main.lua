...
AddEventHandler('iG:playerLoaded', function(playerData)
	ESX.PlayerLoaded = true
	ESX.PlayerData = playerData
	FreezeEntityPosition(PlayerPedId(), false)
	TriggerServerEvent('iG:onPlayerSpawn')
	TriggerEvent('iG:onPlayerSpawn')
	TriggerEvent('iG:loadingScreenOff')
	SetCanAttackFriendly(PlayerPedId(), true, false)
	NetworkSetFriendlyFireOption(true)
end)

RegisterNetEvent('iG:setMaxWeight')
AddEventHandler('iG:setMaxWeight', function(newMaxWeight)
	ESX.PlayerData.maxWeight = newMaxWeight
end)

AddEventHandler('playerSpawned', function()
	-- if isFirstSpawn then
	-- 	TriggerServerEvent('iG:playerJoined')
	-- end
...
AddEventHandler('iG:onPlayerDeath', function() isDead = true end)
AddEventHandler('iG_skinchanger:loadDefaultModel', function() isLoadoutLoaded = false end)

AddEventHandler('iG_skinchanger:modelLoaded', function()
	while not ESX.PlayerLoaded do
		Citizen.Wait(1)
	end
...
AddEventHandler('iG:restoreLoadout', function()
...
AddEventHandler('iG:addInventoryItem', function(item, count, showNotification)
...
AddEventHandler('iG:removeInventoryItem', function(item, count, showNotification)
	for k,v in ipairs(ESX.PlayerData.inventory) do
		if v.name == item then
...
RegisterNetEvent('iG:addWeapon')
AddEventHandler('iG:addWeapon', function(weaponName, ammo)
	local playerPed = PlayerPedId()
	local weaponHash = GetHashKey(weaponName)
...
RegisterNetEvent('iG:addWeaponComponent')
AddEventHandler('iG:addWeaponComponent', function(weaponName, weaponComponent)
	local playerPed = PlayerPedId()
	local weaponHash = GetHashKey(weaponName)
...
RegisterNetEvent('iG:setWeaponAmmo')
AddEventHandler('iG:setWeaponAmmo', function(weaponName, weaponAmmo)
	local playerPed = PlayerPedId()
	local weaponHash = GetHashKey(weaponName)
...
RegisterNetEvent('iG:setWeaponTint')
AddEventHandler('iG:setWeaponTint', function(weaponName, weaponTintIndex)
	local playerPed = PlayerPedId()
	local weaponHash = GetHashKey(weaponName)
...
RegisterNetEvent('iG:removeWeapon')
AddEventHandler('iG:removeWeapon', function(weaponName, ammo)
	local playerPed = PlayerPedId()
	local weaponHash = GetHashKey(weaponName)
...
RegisterNetEvent('iG:removeWeaponComponent')
AddEventHandler('iG:removeWeaponComponent', function(weaponName, weaponComponent)
	local playerPed = PlayerPedId()
	local weaponHash = GetHashKey(weaponName)
	local componentHash = ESX.GetWeaponComponent(weaponName, weaponComponent).hash
...
RegisterNetEvent('iG:teleport')
AddEventHandler('iG:teleport', function(coords)
	local playerPed = PlayerPedId()
...
RegisterNetEvent('iG:spawnVehicle')
AddEventHandler('iG:spawnVehicle', function(vehicle)
...
-- RegisterNetEvent('iG:createPickup')
-- AddEventHandler('iG:createPickup', function(pickupId, label, playerId, type, name, components, tintIndex)
...
-- RegisterNetEvent('iG:createMissingPickups')
-- AddEventHandler('iG:createMissingPickups', function(missingPickups)
...
-- RegisterNetEvent('iG:removePickup')
-- AddEventHandler('iG:removePickup', function(id)
-- 	ESX.Game.DeleteObject(pickups[id].obj)
-- 	pickups[id] = nil
-- end)

RegisterNetEvent('iG:deleteVehicle')
AddEventHandler('iG:deleteVehicle', function(radius)
	local playerPed = PlayerPedId()
...
-- 			TriggerServerEvent('iG:updateLoadout', loadout)
-- 		end
-- 	end
-- end)
...
-- 							TriggerServerEvent('iG:onPickup', v.id)
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if distance > 10 then
			...
			TriggerServerEvent('iG:updateCoords', formattedCoords)
		end
	end
end)
