                            ...
                            if UTK.checks.id1 and not UTK.checks.id2 then
                                ...
                                if dst <= 4 then
                                    ...
                                    if dst <= 1 and IsControlJustReleased(0, 38) then
                                        ESX.TriggerServerCallback("utk_oh:checkItem", function(output)
                                        if output then
                                            TriggerServerEvent("utk_oh:removeitem", "gruppecard")
                                            UTK.checks.id2 = true
                                            self.currentid = 2
                                            self:IdCard()
                                        elseif not output then
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            if UTK.begingas then
                ...
                UTK.stage2break = true
                UTK.stage4break = true
                exports["mythic_notify"]:SendAlert("error", "Heist: Pacific Standard Bank's vault door is beginning to close.")
...
AddEventHandler("utk_oh:policenotify", function(toggle)
    ...
    if player.job.name == "police" then
        if toggle == 1 then
            exports["mythic_notify"]:SendAlert("infrom", "Heist: Pacific Standard Bank's alarms have been triggered.", 10000, {["background-color"] = "#CD472A", ["color"] = "#ffffff"})
...
Citizen.CreateThread(function() while ESX == nil do TriggerEvent('iG:getSharedObject', function(obj) ESX = obj end) Citizen.Wait(1) end Citizen.Wait(10000) UTK:GetInfo() end)
